...
- Токены захардкожены в коде
- Всё в одном файле (монолит)
- SQLite без async-драйвера блокирует event loop
- `MOVIE_CACHE` без TTL — растёт бесконечно
- Cloudflare Tunnel меняет URL при каждом рестарте
